...
function buildServiceQuery(params: URLSearchParams) {
  const order = params.get('order')
  const direction = params.get('direction') === 'asc' ? 'ASC' : 'DESC'
  let text = 'SELECT * FROM services'
  const values: any[] = []
...
export async function GET(req: NextRequest) {
  try {
    const { text, values } = buildServiceQuery(req.nextUrl.searchParams)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const text = `INSERT INTO services (${columns
      .map((column) => `"${column}"`)

...
const allowedOrderColumns = ['created_at', 'preferred_date']

function buildAppointmentQuery(params: URLSearchParams) {
  const order = params.get('order')
  const direction = params.get('direction') === 'asc' ? 'ASC' : 'DESC'
  let text = 'SELECT * FROM appointments'
  const values: any[] = []
...
export async function GET(req: NextRequest) {
  try {
    const { text, values } = buildAppointmentQuery(req.nextUrl.searchParams)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const text = `INSERT INTO appointments (${columns
      .map((column) => `"${column}"`)
